...
class Spotify_UWP {
    ; Match Spotify executable OR any major chromium browser
    static winExe := "(Spotify|chrome|brave|vivaldi|msedge|opera)\.exe"
    static winTitle => WinGetTitle("ahk_exe " this.winExe)
...
;        this._DisplayOsd(LanguagePack[General.CurrentLang]["Previous"])
    ...
    static ToggleMute() {
        ...
        spotifyHwnd := WinExist("ahk_exe vivaldi.exe")
        if (spotifyHwnd) {
            try {
        ...
        isSpotifyActive := WinActive("ahk_exe vivaldi.exe")
    ...
    static OpenSpotify() {
        ...
        targetWindow := "ahk_exe " this.winExe
        
        try {
            ; Only attempt to launch the desktop app if no player is running anywhere
            ...
            MsgBoxCustom("Could not connect to a Spotify session.", App.Name)
        }
    ...
    static _GetCorrectHwnd() {
        ...
        for hwnd in hwndList {
            ...
            if (title != "") {
                ; Verify if this specific window handle contains Spotify's web player framework
                try {
                    if UIA.ElementFromHandle(hwnd).FindElement({ AutomationId: "RootWebArea" }) {
                        correctHwnd := hwnd
                        break
                    }
                } catch {
        ...
        DetectHiddenWindows(oldSetting)
        SetTitleMatchMode(oldMatchMode)
        return correctHwnd ? correctHwnd : WinExist("ahk_exe " this.winExe)
    }
}