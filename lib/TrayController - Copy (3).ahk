...
global AnimationDuration := 80  
global FrameRate := 10          

; --- CURSOR UTILITIES ---
; Load the standard system "Hand" pointer cursor
...
OnMessage(0x0020, WM_SETCURSOR) ; <--- Intercept WM_SETCURSOR
...
WM_SETCURSOR(wParam, lParam, msg, hwnd) {
    global hCursorHand
    ; Check if the targeted control window belongs to one of our active buttons
    for ctrl in ClickableCtrls {
        if (ctrl.Hwnd == wParam && ctrl.Visible) {
            DllCall("SetCursor", "Ptr", hCursorHand)
            return 1 ; Tell Windows we handled it, stop changing back to standard arrow
        }
    }
}
